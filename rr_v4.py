...
No warrenties of accuray for risk range is implied. 
Links are provided for core functions for your review.
As you enhance the code, open source your work with the group. 
Jim Spearman'''
...
try:
    c = auth.client_from_token_file('./token', config.TD_API_KEY)
except FileNotFoundError:
    from selenium import webdriver
    with webdriver.Chrome(executable_path='./chromedriver.exe') as driver:
        c = auth.client_from_login_flow(driver, config.TD_API_KEY, config.TD_URL, config.TD_TOKEN)
